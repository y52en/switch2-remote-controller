...
static int gatt_svc_write_no_rsp_access(uint16_t conn_handle, uint16_t attr_handle,
  ...
  if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
    if (attr_handle == gatt_svr_chr_0016_val_handle) {
      ...
      // Copy only the bounded Pro Controller 2 vibration envelope. Decoding and
      // USB delivery are latest-value-wins and run outside the NimBLE callback.
      uint16_t data_len = os_mbuf_len(ctxt->om);
      size_t copy_len = data_len < RUMBLE_FEEDBACK_MAX_PACKET_SIZE
